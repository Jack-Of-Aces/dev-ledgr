  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 md:py-16 space-y-10">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-radius border border-line bg-card text-text-0 placeholder:text-text-1 focus:border-green-500 outline-none text-xs md:text-sm"
            />
          </div>
          ...
          >
            {domains.map((d) => (
              <button
                ...
                className={`px-3 py-1.5 transition-colors cursor-pointer text-xs md:text-sm font-sans whitespace-nowrap border-b-2 ${
              ...
              >
                <span>{d.label}</span>
                {d.count !== undefined && (
                  <span className="ml-1.5 opacity-70 text-xs md:text-sm font-mono">({d.count})</span>
                )}
              </button>
            ))}
          </div>
      ...
      {filteredIdeas.length > 0 ? (
            ...
            return (
              ...
              >
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 text-xs md:text-sm font-mono">
                    <div className="flex items-center gap-2">
                      <span className={`px-2.5 py-1 rounded border text-xs md:text-sm font-semibold ${domainStyle.badge}`}>
                        {domainStyle.name}
                      </span>
                ...
                <div>
                  ...
                  <p className="text-xs md:text-sm sm:text-sm lg:text-base text-text-1 mt-2 leading-relaxed">
                    {idea.tagline}
                  </p>
                </div>

                {/* Origin Story Quote - Plain Typographic Style without Nested Card */}
                <p className="text-xs md:text-sm text-text-1 leading-relaxed italic pl-3 border-l-2 border-line">
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {idea.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-xs md:text-sm px-2.5 py-1 rounded border border-line bg-card text-text-1 font-mono"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <div className="pt-4 border-t border-line flex flex-wrap items-center justify-between gap-3 text-xs md:text-sm">
                <span className="text-xs md:text-sm text-green-700 dark:text-green-400 flex items-center gap-1.5 font-medium font-mono">
                  <span className="w-1.5 h-1.5 rounded-full bg-green-600 dark:bg-green-500" />
                  Mock Infra Ready
                </span>
                <Link
                  href={`/ideas/${idea.id}`}
                  className="btn-brass text-xs md:text-sm py-1.5 px-3.5 inline-flex items-center gap-1.5"
                >
        ...
        <div className="p-12 rounded-radius border border-dashed border-line text-center text-xs md:text-sm space-y-3">
          ...
          <button
            ...
            className="btn-outline text-xs md:text-sm py-1.5 px-3 inline-flex items-center gap-1.5"
          >
