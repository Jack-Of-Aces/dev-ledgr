...
export default function DashboardPage() {
  ...
  return (
    <AuthGuard fallbackMessage="Please sign in to access your developer portfolio dashboard.">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 md:py-16 space-y-8 text-sm lg:text-base font-sans">
      {/* Top Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-line">
        <div>
          ...
          <p className="text-text-1 mt-1 text-xs md:text-sm">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-radius border border-line bg-card hover:border-zinc-500 transition-colors cursor-pointer text-xs md:text-sm"
          >
          ...
          <Link
            ...
            className="btn-outline text-xs md:text-sm py-2 px-3.5 inline-flex items-center gap-1.5"
          >
      ...
      <div className="p-4 rounded-radius border border-line bg-card/40 flex flex-col md:flex-row md:items-center justify-between gap-4 text-xs md:text-sm">
        <div className="flex items-start sm:items-center gap-3">
          ...
          <div>
            <div className="font-medium text-text-0 flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs md:text-sm text-green-700 dark:text-green-400 bg-green-500/10 px-2.5 py-1 rounded border border-green-500/20 font-mono font-medium">
                verified online
              </span>
            </div>
            <div className="text-xs md:text-sm text-text-1 mt-0.5 font-mono break-all sm:break-normal">
      ...
      <div className="flex flex-wrap items-center justify-between gap-y-3 gap-x-8 py-3.5 px-4 border border-line bg-card/20 rounded-radius text-xs md:text-sm">
        <div className="flex items-center gap-2">
          ...
          <span className="text-green-700 dark:text-green-400 text-xs md:text-sm font-mono font-medium">(100% CI pass)</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-text-1">Matched Roles:</span>
          <span className="font-semibold text-text-0">{jobs.length} active</span>
          <span className="text-green-700 dark:text-green-400 text-xs md:text-sm font-mono font-medium">(Top: Moniepoint 94%)</span>
        </div>
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/settings" className="text-green-700 dark:text-green-400 hover:underline text-xs md:text-sm ml-1 font-mono font-medium">
            Keys →
          </Link>
        </div>
      </div>
          ...
          >
            ...
            <div className="space-y-3">
              {notifications.map((n) => (
                <div
                  key={n.id}
                  className="p-3.5 rounded-radius border border-line bg-card/50 space-y-1 text-xs md:text-sm"
                >
                  <div className="flex justify-between items-center text-xs md:text-sm text-text-1">
                  ...
                  <p className="text-text-1 text-xs md:text-sm leading-relaxed">{n.desc}</p>
                </div>
              ))}
            </div>
          </div>
