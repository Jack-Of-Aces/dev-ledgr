...
export default function JobsPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 md:py-16 space-y-10">
      ...
      <div className="rounded-radius border border-line bg-card p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
        ...
        <Link
          href="/ideas"
          className="btn-outline text-xs py-1.5 px-3 self-start sm:self-auto shrink-0 font-sans"
        >
          Solve more problems →
        </Link>
      </div>
      ...
      <div className="space-y-4">
        {jobs.map((job) => {
          ...
          return (
            <div
              key={job.id}
              className="rounded-radius border border-line bg-card hover:border-zinc-700/80 transition-all p-6 space-y-4 text-xs"
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-line">
                <div>
                  <div className="flex items-center gap-2 text-xs font-mono text-green-700 dark:text-green-400 font-medium">
                ...
                <div className="flex items-center gap-3 font-mono">
                  <div className="text-right">
                    ...
                    <div className={`text-xs font-semibold ${isHighMatch ? 'text-green-700 dark:text-green-400' : 'text-amber-700 dark:text-amber-400'}`}>
              ...
              {/* Description */}
              <p className="text-xs sm:text-sm text-text-0 leading-relaxed max-w-lg">
                {job.description}
              </p>

              {/* Required Proofs & Match tags */}
              <div className="space-y-2 pt-2">
                <div className="text-xs uppercase tracking-wider text-text-1 font-semibold">
                ...
                <div className="flex flex-wrap gap-2">
                  {job.requiredSkills.map((req, i) => (
                    <span
                      key={i}
                      className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded border border-line bg-ink-0 text-xs text-text-0"
                    >
