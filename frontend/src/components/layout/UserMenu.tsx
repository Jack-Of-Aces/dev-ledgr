...
export const UserMenu: React.FC = () => {
  ...
  return (
    <div className="relative font-mono text-xs" ref={menuRef}>
      ...
      >
        {user.avatarUrl ? (
          <div className="relative w-6 h-6 sm:w-5 sm:h-5 rounded-full overflow-hidden shrink-0 border border-line bg-ink-1">
            <Image
              src={user.avatarUrl}
              alt=""
              fill
          ...
          <div className="w-6 h-6 sm:w-5 sm:h-5 rounded-full bg-ink-1 border border-line flex items-center justify-center font-bold text-[10px] text-brass shrink-0">
        ...
        <ChevronDown className="w-3 h-3 text-text-1 hidden sm:inline shrink-0" />
      </button>
      ...
      {open && (
        <div
          role="menu"
          className="absolute right-0 mt-2 w-64 max-w-[calc(100vw-2rem)] rounded-radius border border-line bg-card p-2 shadow-lg z-50 space-y-1 font-mono text-xs animate-in fade-in slide-in-from-top-1 duration-150"
        >
          {/* User Identity Header with Proof Count */}
          <div className="px-3 py-2.5 border-b border-line space-y-1.5">
            <div className="flex items-center justify-between gap-2">
              <span className="font-bold text-text-0 truncate text-xs">{user.name}</span>
              <span
                className={`text-[10px] uppercase px-1.5 py-0.5 rounded font-medium border border-line shrink-0 ${
                  isAdmin ? 'text-text-0 bg-ink-1' : 'text-text-1'
                }`}
              >
            ...
            <div className="flex items-center justify-between text-xs text-text-1">
              ...
              <span className="text-[11px] font-mono text-text-1 bg-ink-0 px-2 py-0.5 rounded border border-line shrink-0">
                {userSubmissions.length} {userSubmissions.length === 1 ? 'proof' : 'proofs'}
              </span>
            </div>
          </div>
